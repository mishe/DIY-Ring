...
module.exports = function drawRing(opts) {
    ...
    function fillBG() {
        ...
        ctx.lineWidth = 2;
        ctx.strokeStyle = color[0];
        ctx.arc(width / 2, width / 2, radius-arc+2, 0, 2 * Math.PI);
        ctx.stroke();
    }

    function fillArc(x) {
        var grd = ctx.createRadialGradient(width / 2, width / 2, 0, width,width, width);
        grd.addColorStop(0, '#ffb05c');
        grd.addColorStop(1, '#eb322d');
        ctx.beginPath();
        ctx.lineWidth = arc;
        ctx.strokeStyle = grd;
        ...
        ctx.lineWidth = 2;
        ctx.strokeStyle = grd;
        ctx.arc(width / 2, width / 2, radius-arc+2, -90 * Math.PI / 180, (x * 3.6 - 90) * Math.PI / 180);
        ctx.stroke();
    }
